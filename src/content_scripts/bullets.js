...
function makeBulletApp(
  ...
  const App = () => {
    ...
    useEffect(() => {
      ...
      return () => {
        if (ytlBulletContentRef.current) {
    ...
    }, [operated, ytlBulletRef, ytlBulletContentRef]);

    // let color = `#${Math.floor(Math.random() * (2 << 23)).toString(16)}`;
    return (
      <div
        className="notranslate bluesea-bullet"
        translate="no"
        ref={ytlBulletRef}
        style={{
          ['WebkitAnimationPlayState']: animationRunning ? 'running' : 'paused',
          boxShadow: animationRunning ? 'none' : '0 0 8px rgba(0, 0, 0, 0.4)',
...
class ButtelScreens {
  ...
  async makeBullet() {
    ...
    makeBulletApp(buttelRoot, {
      ...
      destroy: () => {
        unmountComponentAtNode(buttelRoot);
